...
function LoadSearchImages(get) {
    const classes = useStyles();


    const { data, loading, more, load, photos, searchByCategoryAndKeyword } = useContext(GalleryContext);
    ...
    const observer = useRef(
        new IntersectionObserver((entries) => {
            ...
            const keyword = get.props.match.params.keyword

            const result = searchByCategoryAndKeyword(keyword, category);
            

            if (first.isIntersecting) {
                loader.current(result);
            }
